...
def mergeDF(copy_path):
    ...
    group_confirmed = full_df_trim.groupby(['Country_Region', 'Date']).agg({'Confirmed': ['sum']})
    group_deaths = full_df_trim.groupby(['Country_Region', 'Date']).agg({'Deaths': ['sum']})
    group_recovered = full_df_trim.groupby(['Country_Region', 'Date']).agg({'Recovered': ['sum']})
...
data_path = ("path/to/copy/repo/COVID19_REPO/" + 
    "csse_covid_19_data/csse_covid_19_daily_reports")

copy_data_path = "path/to/daily/data"

deleteOld(os_path, folder_name, data_name)
cloneRepo(git_url, os_path, folder_name)
copyDaily(data_path, copy_data_path)
df = mergeDF(copy_data_path)
plotMetric(df, 'US', 'Confirmed')